...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
  echo json_encode(read_data(), JSON_UNESCAPED_UNICODE);
  exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $findIndex = function($id) use ($items) {
    foreach ($items as $i => $it) { if (($it['id'] ?? null) === $id) return $i; }
    return -1;
  };
  ...
  if ($action === 'upsert') {
    ...
    if (!$item || !isset($item['id'])) {
      http_response_code(400); echo json_encode(['error'=>'Missing item or id']); exit;
    }
    // Met à jour lastReadAt si progress change
    $idx = $findIndex($item['id']);
    if ($idx >= 0) {
      $existing = $items[$idx];
      $prevProgress = $existing['progress'] ?? null;
      $newProgress = $item['progress'] ?? $prevProgress;
